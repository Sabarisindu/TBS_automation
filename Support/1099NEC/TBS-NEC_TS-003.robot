...
TBS-NEC_TS-003
	...
	${TS_id}=       Get cell value    4    B   1099_Nec_Scenario
	${TS_desc}=       Get cell value    4    Z   1099_Nec_Scenario
	Open Workbook      ${wbook}
	...
	${passed3} =    Run Keyword And Return Status      Wait Until Element Is Enabled       ${obj_saveadd}     5s
	...
	Run Keyword	Capture Page Screenshot     ../Support/Screenshots/${TS_id}.png
  	...
  	Run Keyword If    ${passed3}      writelog    '${logid}','${TS_id}','${TS_desc}','NEC form should be created with all input values','NEC form created with all input values','Pass','${TS_id}.png' 
	...        ELSE		writelog    '${logid}','${TS_id}','${TS_desc}','NEC form should be created with all input values','NEC form created with all input values','Fail','${TS_id}.png'
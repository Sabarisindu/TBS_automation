...
TBS-NEC_TS-001
	...
	${TS_id}=       Get cell value    2    B   1099_Nec_Scenario
	${TS_desc}=       Get cell value    2    Z   1099_Nec_Scenario
	Open Workbook      ${wbook}
	...
	Run Keyword	Capture Page Screenshot		../Support/Screenshots/${TS_id}.png
  	...
  	Run Keyword If	${passed1}      writelog    '${logid}','${TS_id}','${TS_desc}','NEC form should be created with USPS address','NEC form is created with USPS address','Pass','${TS_id}.png'
 	...        ELSE		writelog    '${logid}','${TS_id}','${TS_desc}','NEC form should be created with USPS address','NEC form is not created with USPS address','Fail','${TS_id}.png'
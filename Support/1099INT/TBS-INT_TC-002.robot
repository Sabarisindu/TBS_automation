...
TBS-INT_TC-002
	...
	${TS_desc}=       Get cell value    3    AS   1099_INT_Scenario
	Open Workbook      ${wbook}
	Wait Until Element is Enabled	${obj_clear}	20s
	Click Element		${obj_clear}
	${Businessdata}=       Get cell value    6    A   1099_INT_Scenario
	Click Element           	${obj_selectpayer}		
	sleep 	10s			
	Input Text			${obj_psearch}			${Businessdata}		
	Click Element        		${obj_spayer}
	${obj_rtype1}=	Get cell value    3    C   1099_INT_Scenario
	${obj_interest1}=	Get cell value    3    Y   1099_INT_Scenario
	...
	Run Keyword If    ${passed2}      writelog    '${logid}','${TS_id}','${TS_desc}','INT form should be created with foreign address','INT form created with foreign address','Pass','${TS_id}.png' 
	...        ELSE		writelog    '${logid}','${TS_id}','${TS_desc}','INT form should be created with foreign address','INT form created with foreign address','Fail','${TS_id}.png'